...
// The multiply hash above is fine for value noise but shows a faint
// axis-aligned mesh at integer fragment coords, which reads as a net
// over flat areas.
...
vec3 linearToSrgb(vec3 c) {
  // max() guards the sRGB branch: out-of-gamut OKLab interpolations can
  // send a channel negative, and pow(negative, …) is NaN which mix()/step()
  // would then propagate. The linear branch clips such channels to 0 downstream.
  return mix(
    c * 12.92,
...
vec3 mixColour(vec3 a, vec3 b, float t) {
  if (u_oklab > 0.5) {
    ...
    return clamp(
      linearToSrgb(oklabToLin(mix(la, lb, t))),
      0.0,
      1.0
    );
  }
...
void main() {
  ...
  // Cursor modes 1–3 are local distortions.
  // Push shifts the same screen-space coordinates before field transforms,
  // so Zoom/Rotate don't change its feel.
  if (u_cursorPresence > 0.001) {
    // u_mouse is normalized to -1..1 in canvas space.
    // Convert it to the same aspect-corrected screen space as p
    // so effects stay under the cursor.
    vec2 cursor =
...
const pendingContextReleases =
  new WeakMap<HTMLCanvasElement, number>()
